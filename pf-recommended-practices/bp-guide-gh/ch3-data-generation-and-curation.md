...
I/O contention, see the [HPC section below](#label-hpc-environments).
...
data writes is a trade-off between IO (input/output) efficiency, data
...
write frequencies and data chunking strategies. I/O performance can be
a major bottleneck for larger parallel simulations, but there are
tools to help characterize I/O, see {cite}`Ather2024` for a thorough
overview.
