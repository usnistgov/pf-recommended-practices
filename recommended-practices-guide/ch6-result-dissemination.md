...
data. There are many general guidelines and recommendations that should be
...
Data for postprocessing is often generated by research codes and
then used for plotting. Plots can be generated and then edited manually or
scripts can be used that define all aspects of the plot.
The use of scripts may increase the time required for an initial plot, but will 
reduce the time required for subsequent regeneration of the plot. Scripts also
ensure that your plots have a consistent style when regenerated.
...
results, it is important to validate and verify the accuracy of the model and its
...
(see {ref}`label-code-verification`). Key
...
you to publish data associated with your publication. There are several
